...
データ収益化管理では、品質改善・AI学習・外部提供の目的別に、同意済みの場合だけ `data_exports` へ輸出リクエストを保存できる。保持期間は `data_retention_policies`、輸出監査は `export_audit_events` に保存する。ユーザー管理画面から同意撤回を保存でき、最新レコードが撤回なら将来の輸出はRLSで拒否される。

Supabase Storageにはプライベートバケット `ikoeru-audio`、`ikoeru-avatars`、`ikoeru-exports` を作成済み。ファイルパス先頭を `tenant_id/` にして、所属テナントだけ読めるRLSを入れている。

OBS画面は左側を外部コメントビューア用に空け、右側にイコエルAI、吹き出し、瞬き、口パクを出す。画像パスはGitHub Pagesの絶対パスとrawフォールバックを持つ。

OBSや外部ジョブが直接呼べる公開APIは、サービスロールを使う実装が安全判定で拒否されたため未公開。代わりに、テナントユーザーRLS、顧客変更依頼、学習許可、同意撤回、データ輸出管理、保持期間、輸出監査、プライベートストレージ、テナント/ストリーム別アクセストークンの土台を入れた。
...
- 顧客の学習・外部提供許可/撤回保存API作成済み。
- `data_exports` と最新同意ゲート付き輸出リクエストAPI作成済み。
- `data_retention_policies` と保持期間API作成済み。
- `export_audit_events` と輸出監査API作成済み。
- プライベートStorageバケット作成済み。
- Storageファイルのテナント別RLS作成済み。
- データ収益化管理画面作成済み。
...
- 音声、アバター、輸出ファイルをプライベートStorageへ保存できる。
